...
public class DynamicDataDaoImpl implements DynamicDataDao{
	...
	@Override
	public List<DynamicData>selectDynamicDataByRadarId(Integer RadarId,String startTimeDate,String endTimeDate){
		EntityManager em = emf.createEntityManager();
		String selectSql = "select * from dynamic_data where radar_id = '"+RadarId+"'and collect_date between '"+startTimeDate+"'and '"+endTimeDate+"'";
	...
	@SuppressWarnings("unchecked")
	@Override
	public	List<DynamicData> selectDynamicDataByManagerId(Integer managerId, String startTimeDate, String endTimeDate){
		EntityManager em = emf.createEntityManager();
		String selectSql = "select * from dynamic_data where radar_id in(select radar_id from radar where manager_id ='"+managerId+"') and "
							+ "collect_date between '"+startTimeDate+"'and '"+endTimeDate+"'";
    Query query = em.createNativeQuery(selectSql,DynamicData.class);
		List<DynamicData> list = query.getResultList();
		em.close();
		return list;
